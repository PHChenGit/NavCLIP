...
class SatelliteImageProcessor:
    def __init__(self, tif_path):
        ...
        self.transform = None
        self.crs = None
        self.crs_mercator = CRS('epsg:3857')
    ...
    def _load_dataset(self):
        ...
        try:
            ...
            # self.transformer = Transformer.from_crs(self.crs, self.crs_mercator, always_xy=True)
            ...
            print(f"  TIF size(width, height): {self.dataset.width}, {self.dataset.height}")
    ...
    def convert_crs(self, src_crs, dst_crs, lat, lon):
    ...
    def gps_to_pixel(self, lat, lon):
        """
        使用 rasterio 將 GPS 座標轉換為像素座標，並自動處理 CRS 轉換。
        ...
        Returns:
            tuple: (col, row) 像素座標 (x, y)，分別對應列與行。
        """
        ...
        print(f"GPS ({lat:.6f}, {lon:.6f}) -> Projected Coords ({dst_crs}): ({x_coord:.2f}, {y_coord:.2f})")
        print(f"Projected Coords -> Pixel (Row, Col): ({row}, {col})")
    ...
    def convert_image_to_pillow(self, image_data) -> Image.Image:
        """
        Convert image format from tif to pillow Image
        """
        # 轉換為Pillow圖像
